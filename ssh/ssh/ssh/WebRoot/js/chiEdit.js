...
if (newtxt != txt) {
	...
	var url = "news_chiupdateNews";
	var codeNewTxt = encodeURIComponent(newtxt);
	//使用get()方法打开一个一般处理程序，data接受返回的参数（在一般处理程序中返回参数的方法 context.Response.Write("要返回的参数");）
	//数据库的修改就在一般处理程序中完成
	$.get(url,{ chiConx: codeNewTxt}, function(data) {
		if(data=="yes")
		{
			alert("新闻更新成功！");
			d.html(newtxt);
...
$("#contact").click(function() {
	var td = $(this);
	var txt = td.text();
	var input = $("<textarea id='contact' cols='60' rows='15' class='contact' style='margin-top:20px'>"+txt+"</textarea>");
	td.html(input);
	input.click(function() { return false; });
	//获取焦点
	input.trigger("focus");
