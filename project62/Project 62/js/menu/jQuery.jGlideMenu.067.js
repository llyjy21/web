...
jQuery.jGlideMenu = {
	...
	initialize : function(o)
	{
		// Return jQuery
			...
			// Ensure Values (Basic)
            if(s.closeLinkMarkUp.length<1) {
                 s.closeLinkMarkUp = 'x Close';
            };
			...
			// <-- Create Menu Structure
             jQuery(jQuery.jGlideMenu.currentElement).append('<div class="jGM_header"><a href="#"><img src="' +jQuery.jGlideMenu.imagePath+'close.png" height="12" width="12"/></a></div>')
	...
	loadTile : function(u,p)
	{
		...
		jQuery('#'+ptr.id+' div.jGM_content a').bind('click',function() {
				...
				if(target.substr(0,1)=='#' && target.substr(0,4)!='#geo')
				{

					if(jQuery.jGlideMenu.animation == true) return false;
                     var array = target.substr(1).split(',');
                                                     
                     var dest = array[0];
                     var target_ligne = array[1].substr(1);
                     ...
                     d3.selectAll("#" + target_ligne).style("opacity", newOpacity);
                     //d3.selectAll("path[name=s" + target_ligne + "]").style("opacity", newOpacity);
                     // Update whether or not the elements are active
                     d3.selectAll("#" + target_ligne).classed("active", active = newOpacity);
					...
					return false;
                }else if(target.substr(0,1)=='#' && target.substr(0,4)=='#geo'){
                             var array = [];
                             var lat,lont;
                             array = target.substr(4).split(',');
                             lat = array[0];
                             longt = array[1];
                             map.setView(new L.LatLng(lat, longt),17);
